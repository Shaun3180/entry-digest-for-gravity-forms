 ...
 */
function edfgf_render_editor( string $action, string $notice ): void {
	...
	$forms_age     = edfgf_forms_age_days( $d['form_ids'] );
	...
	<div class="wrap">
		...
		<form method="post" action="<?php echo esc_url( $base_url . '&action=' . $action . ( $d['id'] ? '&digest=' . rawurlencode( $d['id'] ) : '' ) ); ?>">
			<?php wp_nonce_field( 'edfgf_save_digest' ); ?>
			<input type="hidden" name="edfgf_digest[id]" value="<?php echo esc_attr( $d['id'] ); ?>">
			...
			<div style="<?php echo esc_attr( $card_style ); ?>">
				...
				<table class="form-table" role="presentation"><tbody>
					...
					<tr>
						<th scope="row"><label for="edfgf_label"><?php esc_html_e( 'Digest name', 'entry-digest-for-gravity-forms' ); ?></label></th>
						<td><input type="text" id="edfgf_label" name="edfgf_digest[label]" value="<?php echo esc_attr( $d['label'] ); ?>" class="regular-text">
					...
					<tr>
						...
						<td>
							...
							<?php else : ?>
								<input type="number" name="edfgf_digest[form_ids][]" value="<?php echo esc_attr( $d['form_ids'][0] ?? 1 ); ?>" min="1" class="small-text">
			...
			<div style="<?php echo esc_attr( $card_style ); ?>">
				...
				<table class="form-table" role="presentation"><tbody>
					...
					<tr>
						<th scope="row"><label for="edfgf_to_email"><?php esc_html_e( 'Recipient email(s)', 'entry-digest-for-gravity-forms' ); ?></label></th>
						<td><textarea id="edfgf_to_email" name="edfgf_digest[to_email]" rows="2" class="large-text"><?php echo esc_textarea( $d['to_email'] ); ?></textarea>
					 ...
					 */
					do_action( 'edfgf_editor_after_recipients', $d );
					?>

					<tr>
						<th scope="row"><label for="edfgf_subject"><?php esc_html_e( 'Email subject', 'entry-digest-for-gravity-forms' ); ?></label></th>
					 ...
					 * @param array $d The digest configuration being edited.
					 */
					do_action( 'edfgf_editor_email_options', $d );
					?>
			...
			<div style="<?php echo esc_attr( $card_style ); ?>">
				...
				<table class="form-table" role="presentation"><tbody>
					...
					<tr>
						<th scope="row"><label for="edfgf_freq"><?php esc_html_e( 'Recurring frequency', 'entry-digest-for-gravity-forms' ); ?></label></th>
						<td>
							<select id="edfgf_freq" name="edfgf_digest[frequency]">
					...
					<tr class="edfgf-weekly-row">
						<th scope="row"><label for="edfgf_day"><?php esc_html_e( 'Send day', 'entry-digest-for-gravity-forms' ); ?></label></th>
						<td><select id="edfgf_day" name="edfgf_digest[send_day]">
							<?php foreach ( $days as $day ) : ?>
								<option value="<?php echo esc_attr( $day ); ?>" <?php selected( $d['send_day'], $day ); ?>><?php echo esc_html( edfgf_day_label( $day ) ); ?></option>
							<?php endforeach; ?>
						</select> <span class="description"><?php esc_html_e( 'Weekly only.', 'entry-digest-for-gravity-forms' ); ?></span></td>
					</tr>

					<tr class="edfgf-recurring-row">
						<th scope="row"><label for="edfgf_time"><?php esc_html_e( 'Send time', 'entry-digest-for-gravity-forms' ); ?></label></th>
						<td><input type="time" id="edfgf_time" name="edfgf_digest[send_time]" value="<?php echo esc_attr( $d['send_time'] ); ?>">
							<p class="description"><?php
					...
					<tr>
						<th scope="row"><label for="edfgf_onetime"><?php esc_html_e( 'One-time send', 'entry-digest-for-gravity-forms' ); ?></label></th>
						<td>
							<input type="datetime-local" id="edfgf_onetime" name="edfgf_digest[onetime_at]" value="<?php echo esc_attr( $onetime_input ); ?>" min="<?php echo esc_attr( $onetime_min ); ?>">
							<?php if ( $onetime_input ) : ?>
								<button type="button" class="button button-small" id="edfgf_onetime_clear"><?php esc_html_e( 'Clear', 'entry-digest-for-gravity-forms' ); ?></button>
							<?php endif; ?>
							<p class="description"><?php
								/* translators: %s: site timezone string, e.g. America/Denver. */
					...
					<tr class="edfgf-onetime-row">
						<th scope="row"><label for="edfgf_lookback"><?php esc_html_e( 'One-time lookback (days)', 'entry-digest-for-gravity-forms' ); ?></label></th>
						<td>
							<input type="number" id="edfgf_lookback" name="edfgf_digest[onetime_lookback_days]" value="<?php echo esc_attr( $lookback_val ); ?>" min="0" step="1" class="small-text">
							<p class="description">
								<?php
					...
					<tr>
						<th scope="row"><label for="edfgf_quiet"><?php esc_html_e( 'When there are no new entries', 'entry-digest-for-gravity-forms' ); ?></label></th>
						<td>
							<select id="edfgf_quiet" name="edfgf_digest[quiet_behavior]">
			...
			<div style="<?php echo esc_attr( $card_style ); ?>">
				...
				<table class="form-table" role="presentation"><tbody>
					...
					<tr>
						...
						<td>
							<div id="edfgf-count-preview" aria-live="polite" style="font-size:14px;line-height:1.5;min-height:22px;">
				...
				foreach ( $all_forms as $form ) :
					...
					$field_map = edfgf_build_field_map( $form );
					$sel_fields = (array) ( $d['fields'][ $fid ] ?? [] );
					?>
					<div class="edfgf-form-block" data-fid="<?php echo esc_attr( $fid ); ?>" style="<?php echo $selected ? '' : 'display:none;'; ?>border:1px solid #dcdcde;border-radius:6px;padding:14px 18px;margin:0 0 14px 0;background:#fbfbfc;">
						 ...
						 */
						do_action( 'edfgf_editor_form_block', $fid, $d, $field_map );
						?>
					</div>
				<?php endforeach; ?>
			</div>

			<?php submit_button( 'new' === $action ? __( 'Create Digest', 'entry-digest-for-gravity-forms' ) : __( 'Save Digest', 'entry-digest-for-gravity-forms' ), 'primary', 'edfgf_save_digest' ); ?>
		</form>
		...
		<div style="<?php echo esc_attr( $card_style ); ?>">
			...
			<?php if ( $d['id'] ) : ?>
				<table class="form-table" role="presentation"><tbody>
					<tr>
						<th scope="row"><label for="edfgf_test_email"><?php esc_html_e( 'Send a test to', 'entry-digest-for-gravity-forms' ); ?></label></th>
						<td>
							<form method="post" action="<?php echo esc_url( $base_url . '&action=edit&digest=' . rawurlencode( $d['id'] ) ); ?>" style="margin:0;">
								<?php wp_nonce_field( 'edfgf_send_test' ); ?>
								<input type="hidden" name="digest_id" value="<?php echo esc_attr( $d['id'] ); ?>">
								<input type="email" id="edfgf_test_email" name="test_email" value="<?php echo esc_attr( wp_get_current_user()->user_email ); ?>" class="regular-text" placeholder="you@example.com">
								<button type="submit" name="edfgf_send_test" class="button"><?php esc_html_e( 'Send test', 'entry-digest-for-gravity-forms' ); ?></button>
